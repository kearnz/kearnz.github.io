...
```{r plotsforsingle, include=TRUE, echo=FALSE}
library(ggplot2)
ggplot(data.frame(weight = c(40,110)), aes(x = weight)) +
        stat_function(fun = dnorm, args = list(60, 5), aes(colour = "Soft")) +
        stat_function(fun = dnorm, args = list(90, 5), aes(colour = "Hard")) + 
        scale_x_continuous(name = "Weight") +
        scale_y_continuous(name = "Density") + 
        ggtitle("Probability Density for Weights on a Soft vs. Hard scale") + 
        scale_colour_manual("Scale Surface", values = c("dodgerblue3", "deeppink"))
```
