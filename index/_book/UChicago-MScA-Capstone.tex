...
missing data is the same (Van Buuren, 2018, ch.~2.2.6). Thus, the
researcher can focus on the impact of the scale surface alone, as
...
\end{itemize}
The sections below define and explore imputation models. In doing so,
the authors reference the effect a given imputation model may have when
subsequent analysis is performed. That being said, the researchers
devote an entire section to analysis models after a full review of
...
\section*{Single Imputation}\label{background-single-imputation}
...
standard error estimates for parameter coefficients that tend to be too
low (Gelman \& Hill, 2017, pg. 532). Gelman (2017, pg. 532) expands upon
