...
One must understand what missingness means and why it exists before
deciding how to deal with it. The authors explore concepts related to
...
scales. Each weigh scale in the experiment may fail to produce weight
measurements for three separate reasons. First, a scale might run out of
batteries, in which case all measurements from that scale cease for a
given period of time. Next, a scale may fail more frequently for heavier
objects or items over a certain weight threshhold. And finally, a scale
may stop reporting measurements when placed on a soft surface instead of
a hard one (Van Buuren, 2018, ch.~1.2).
...
exploring missingness and how to deal with it.
...
also introduce some basic terminology.
...
hand, contains records with missing observations across any of column
\(Y_j\) in \(p\). Note that \(Y = (Y_{obs}, Y_{mis})\) - the
...
This research follows the notation Van Buuren uses in the second edition
of his book, Flexible Imputation of Missing Data. For more information,
refer to Appendix B, which collects and summarizes the notation seen
...
cited in Van Buuren, 2018, ch.~1.2). As a process, the missing data
mechanism generates a statistical relationship between observations and
the probability of missing data (Nakagawa, 2015, pg. 83). The
...
\(\psi\) cointains the parameters of the missing data model. Therefore,
this equation notes that the probability of missingness \(P(R=0)\)
within a dataset depends on the observed data, the missing data, and the
missing data model's parameters. (Van Buuren, 2018, ch.~2.2.4). From
another lense, this equation states that the distribution of missingness
is conditional on the observed data, the missing data, and the
parameters of the missing data model.
...
label. The sections below examine each category in more detail.
...
observed information only (Gelman \& Hill, 2017, pg. 530). Therefore,
